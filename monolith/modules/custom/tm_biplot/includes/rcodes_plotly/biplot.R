...
colTypes    <- chartOptions$x_col_typ
colTypes    <- strsplit(colTypes,",")
rowColumn   <- chartOptions$color_col
rowColumnType   <- chartOptions$color_col_type
...
for (colname in colnames[[1]]) {
  ...
  colsToSelect <- append(colsToSelect, colname)
}
...
colsTypeCoerse <- append(colsTypeCoerse, rowColumnType)
...
if(length(chartOptions$filters) > 0) {
  ...
  dfFiltered <- df[, c(colsToSelect, rowColumn)]
  ...
  if (nrow(dfFiltered) > 0) {
    ...
    tryCatch({
      ...
      ord <- prcomp(dfFiltered[,colsToSelect])
      
      result <- try(
        chartGenerateTimeElapsed <- system.time (
          
          gg <- ggord(ord, dfFiltered[, rowColumn]) + 
            xlab(xTitle) + 
            ylab(yTitle) 
            #ggtitle(chartTitle)
        )
