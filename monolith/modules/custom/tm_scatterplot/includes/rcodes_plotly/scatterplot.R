...
scene = list(xaxis = list(title = xTitle, titlefont = axisf),
             ...
             camera = list(eye = list(x = 1.3, y = 1.7, z = 1.5)))
...
if(length(chartOptions$filters) > 0) {
  ...
  if (colorCol == "emptycol") {
    ...
    if (colorColType == "factor" & (  
      nrow(unique(dfFiltered[colorCol])) < 3 | nrow(unique(dfFiltered[colorCol])) > 10)) {
  ...
  if (nrow(unique(dfFiltered[1])) == 1 | nrow(unique(dfFiltered[2])) == 1 | nrow(unique(dfFiltered[3])) == 1) {
      ...
      if (colorCol == "emptycol") {
        result <- try(
          chartGenerateTimeElapsed <- system.time (
            p <- plot_ly(dfFiltered, x = dfFiltered[[xCol]],
                         y = dfFiltered[[yCol]], z = dfFiltered[[zCol]]) %>% add_markers() %>%
                         config(displaylogo = F, collaborate = F) %>% layout(scene = scene))
        , silent = TRUE)
      } else if (colorColType == "factor") {
        result <- try(
          chartGenerateTimeElapsed <- system.time (
            p <- plot_ly(dfFiltered, x = dfFiltered[[xCol]],y = dfFiltered[[yCol]],
                         z = dfFiltered[[zCol]], color = dfFiltered[[colorCol]], colors = "Paired") %>%
                         add_markers() %>% config(displaylogo = F, collaborate = F) %>%
                         layout(scene = scene) ), silent = TRUE)
      } else {
        result <- try(
          chartGenerateTimeElapsed <- system.time (
            p <- plot_ly(dfFiltered, x = dfFiltered[[xCol]],y = dfFiltered[[yCol]],
                         z = dfFiltered[[zCol]],color = dfFiltered[[colorCol]], colors = "Paired") %>%
                         add_markers() %>% colorbar(title = colorCol) %>%
