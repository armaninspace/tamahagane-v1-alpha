...
# create chart with length of non-empty and non-na columns
ColumnUniqueCount <- function (dataFrame) {
  # get columns names list    
  colnamesVec <- colnames(dataFrame)
  nrows <- nrow(dataFrame)
  
  # get length of non-empty and non-na columns
  statsTable <- as.data.frame(table(dataFrame))
  
  rowCounts <- statsTable$Freq
  rowLabels <- statsTable$dataFrame
  
  # alpha numeric only
  rowLabels <- gsub("[^[:alnum:]///' ]", "", rowLabels) 
  df <- data.frame(rowLabels, rowCounts)
  colnames(df) <- c("labels", "values")
  df
}
...
if(length(chartOptions$filters) > 0) {
  ...
  if (nrow(dfFiltered) > 0) {
    ...
    tryCatch({
      ...
      # dress up data
      df <- ColumnUniqueCount(dfFiltered)
      
      result <- try(
        chartGenerateTimeElapsed <- system.time (
        p <- plot_ly(df, x = ~labels, y = ~values, type = "bar", marker = list(color=colors)) %>%
          layout(xaxis = x, yaxis = y)
        )
        , silent = TRUE)
      
      # p <- plot_ly(x = dfFiltered[[xCol]], type = "bar") %>%
      #   layout(xaxis = x, yaxis = y, title = chartTitle)
