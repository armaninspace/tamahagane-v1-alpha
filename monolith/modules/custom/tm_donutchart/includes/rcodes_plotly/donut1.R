...
# create pie chart with length of non-empty and non-na columns
...
chartTitle <- ifelse(is.null(chartTitle), "Piechart", chartTitle)
  ...
  if (nrow(dfFiltered) > 0) {
    ...
    tryCatch({
      ...
      result <- try(
        ...
        p <- plot_ly(labels = df$labels, values = df$values,
