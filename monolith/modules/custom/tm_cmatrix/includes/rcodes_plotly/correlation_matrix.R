...
if(length(chartOptions$filters) > 0) {
  ...
  for (variable in colnames) {
    ...
    dfFiltered <- dfFiltered[variable]
  }
  ...
  if (nrow(dfFiltered) > 1) {
    ...
    tryCatch({
      ...
      result <- try(
        chartGenerateTimeElapsed <- system.time (
          ...
          gg <- qplot(x=Var1, y=Var2, data=melt(cor(dfFiltered)), fill=value, geom="tile") +
          scale_fill_gradient(low="#A6CEE3", high="#1F78B4") +
            xlab(xTitle) + 
            ylab(yTitle) +
            theme(
              axis.title.x = element_text(family = "Open Sans, verdana, arial, sans-serif", size = 12, color = "#444444"),
              axis.title.y = element_text(family = "Open Sans, verdana, arial, sans-serif", size = 12, color = "#444444"),
              legend.title = element_text(family = "Open Sans, verdana, arial, sans-serif", size = 12, color = "#444444"),
              legend.text = element_text(family = "Open Sans, verdana, arial, sans-serif", size = 8, color = "#444444"),
    ...
    inboundContractCheck <- append(inboundContractCheck, list("output" = FALSE))
    print(toJSON(inboundContractCheck))
    stop("Not enough rows for generating Chart!")
  }
