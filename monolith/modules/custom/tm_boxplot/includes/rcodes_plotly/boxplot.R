...
pal <- c("#4F7EC2", "#CE4E4B", "#9EBF4E", "#7F67A5", "#4DA9CE", "#EF994E", "#6D99DA", "#CD6E6A", "#B9D171", "#9E7DB4", "#66C4E0", "#F7B264", "#81B4EB", "#F08380", "#D1E991", "#B398CD", "#74E1FE", "#FFC97A")
...
if(length(chartOptions$filters) > 0) {
  ...
  if (colorCol == "emptycol") {
    ...
    dfFiltered <- dfFiltered %>%
      select(xCol, colorCol)
    
    # check if color col levels are less than 3.
    if (colorColType == "factor" & (  
      nrow(unique(dfFiltered[2])) < 3 |
      nrow(unique(dfFiltered[2])) > 10)) {
    ...
    # coerce types
    coerceTimeElapsed <- system.time(
      dfFiltered <- convert.magic(dfFiltered, c(xColType, colorColType))
    )
  }
      ...
      if (colorCol == "emptycol") {
        result <- try(
          chartGenerateTimeElapsed <- system.time (
            p <- plot_ly(dfFiltered, y = dfFiltered[[1]],
                         type = "box") %>%
              layout(xaxis = x))
          , silent = TRUE)
      }
        ...
        result <- try(
          chartGenerateTimeElapsed <- system.time (
            p <- plot_ly(dfFiltered, y = dfFiltered[[1]], color = dfFiltered[[2]],
