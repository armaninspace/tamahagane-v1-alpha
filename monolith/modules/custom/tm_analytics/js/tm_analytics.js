...
var isDragging = false;
		...
		mouseup: function() {
			...
			if (wasDragging) {
				tinyMCE.remove();
				$('.analytics-textarea-main .mathjax').show();
				$('.analytics-textarea-main textarea').hide();
				$(".analytics-textarea-main #txt-blocks-controls .glyphicon-edit").show();
				return true;
			}
		}
		}, ".graphical-area .glyphicon-move");
     ...
     */
        ...
        if($('.analytics-textarea').length > 0) {
          ...
          $(wrapper).prepend('<div class="analytics-textarea-main draggable-element" id="text_div_'+divs_length+'"><div id="txt-blocks-controls"><span class="text-block-heading blocks-heading">Paragraph</span><span class="glyphicon glyphicon-move" title="Move"></span><span id="txt-minus-plus" class="glyphicon glyphicon-minus-sign" title="Minimize"></span><span title="Edit" class="glyphicon glyphicon-edit edit_txt_div" id="text_div_'+divs_length+'"></span><span class="glyphicon glyphicon-remove-sign" title="Remove"></span></div><div class="analytics-textarea"><div id="analytics-area-'+divs_length+'" class="mathjax"></div><textarea id="analytics-textarea-'+divs_length+'"></textarea></div></div>'); //add input box
          $("#text_div_"+divs_length+" #txt-blocks-controls .glyphicon-edit, #analytics-area-"+divs_length).hide();
          $('.draggable-element').arrangeable({dragSelector: '.glyphicon-move'});
      ...
      var txtdivid = "text_div_"+divs_length;
			tinyMCE.init({
				...
				theme : "modern",
        menubar: false,
       toolbar: "styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link | save",
        plugins: "mathslate link save",
        ...
        save_onsavecallback: function () {
          ...
          $("#analytics-area-"+divs_length).html(htmlContent).show();
          $("#text_div_"+divs_length+" #txt-blocks-controls .glyphicon-edit").show();
          tinyMCE.remove();
          $("#analytics-textarea-"+divs_length).hide();
        }
			});
		...
		$(wrapper).on("click","#txt-minus-plus", function(e) {
			...
			} else if($(this).parent().parent().attr('class') == "img_div draggable-element") {
    ...
    $("#charts-add-img").click(function () {
      ...
      divs_length = divs_length + 1;
      var html = '<div id="img_div_'+divs_length+'" class="img_div draggable-element">';
      html+=  '<div id="img-blocks-controls"><span class="img-block-heading blocks-heading">Image</span><span class="glyphicon glyphicon-move" title="Move"></span><span id="txt-minus-plus" class="glyphicon glyphicon-minus-sign" title="Minimize"></span><span id="img_div_'+divs_length+'_rem" class="glyphicon glyphicon-remove-sign rem_img_div" title="Remove"></span></div>';
      ...
      $("#img_div_"+divs_length+" .img-body").fineUploader({
        ...
        callbacks: {
          onSubmitted: function(id, name){
            $("#img_div_"+divs_length+" ul.qq-upload-list-selector").hide();
            $("#img_div_"+divs_length+" .qq-uploader-selector").append('<img class="loading_img" src="/sites/default/files/loading.gif">');
          },
          onComplete: function(id, name, responseJSON, xhr){
			  $("#img_div_"+divs_length+" .img-body").empty().html('<img class="fine_uploader_img" src="/sites/default/files/pictures/'+responseJSON.uuid+'/'+name+'">');
    ...
    $(".graphical-area").on("click", '.edit_txt_div', function() {
      ...
      $("#text_div_"+splitID[2]+" #txt-blocks-controls .glyphicon-edit").hide();
      var text_area_id = "analytics-textarea-"+splitID[2];
      var value = $("#analytics-area-"+splitID[2]).html();
      $("#analytics-area-"+splitID[2]).hide();
      $("#analytics-textarea-"+splitID[2]).remove();
      var divid = "text_div_"+splitID[2];
      $("#text_div_"+splitID[2]+" .analytics-textarea").append('<textarea id="analytics-textarea-'+splitID[2]+'"></textarea>');
		tinyMCE.init({
        mode : "textareas",
        theme : "modern",
        ...
        force_p_newlines : false,
        forced_root_block : '',
        selector : "#analytics-textarea-"+splitID[2],
        /* init_instance_callback: function (editor) {
         editor.on('blur', function (e) {
         var htmlContent = tinyMCE.get("analytics-textarea-"+splitID[2]).getContent();
         var txtval = htmlContent.replace(/"/g, "'");
         txtval = txtval.replace(/(?:\r\n|\r|\n)/g, '<br />');
         addtxtDivs(divid, txtval);
         });
         },*/
        save_onsavecallback: function () {
          var htmlContent = tinyMCE.get("analytics-textarea-"+splitID[2]).getContent();
          ...
          addtxtDivs(divid, txtval);
          $("#save_report").trigger("click");
          $("#analytics-area-"+splitID[2]).html(htmlContent).show();
          $("#text_div_"+splitID[2]+" #txt-blocks-controls .glyphicon-edit").show();
          tinyMCE.remove();
          $("#analytics-textarea-"+splitID[2]).hide();
        }
      });
      setTimeout(function(){
        tinyMCE.get("analytics-textarea-"+splitID[2]).setContent(value);
      }, 1000);
      $('#btn-save').addClass('alert-red');
    });
    ...
    */
     ...
     * to clear all charts
     */
	var modalConfirmClear = function(callback){
    ...
    $('body').on('click','#btn-clear-report', function() {
		...
		modalConfirmClear(function(confirm) {
			if (confirm) {
				var imgName = '';
				var imgType = '';
    ...
    */
    ...
    $(".graphical-area").on("click", '.rem-chart', function() {
      ...
      /* $(".analytics-column .xl-report-display-tpl-row").addClass("after_modal_appended");
       ...
       $('body').css("padding-right","");*/
      setModalBackdrop2();
      var delElmntID = $(this).attr('id');
      $("#del-modal #img_id").val(delElmntID);
      modalConfirm(function(confirm) {
        if(confirm){
          var delID = $("#del-modal #img_id").val();
          var splitID = delID.split("_");
	...
	$("body").on("click", '.analytics-side', function() {
      ...
      var textJson = jQuery('#chartsdata #datacharts').val();
        if(textJson){
          ...
          for (var i = obj.length; i--;) {
            if (!obj[i].type) {
              var imgType = obj[i]["ImgType"];
              if (imgType == 'textcluster') {
                var ImgID = obj[i]["ImgID"];
                var splittedID = ImgID.split("_");
                var tclusterImgNum = splittedID[2];
                Imgnames.push({
                  "img_name": obj[i]["ImgName"],
                  "div_id": tclusterImgNum
                });
      ...
      if($(this).hasClass('closed')) {
        ...
        $('.svg-container').width('860');
        $('.nav-tabs a[href="#tm_grid"]').tab('show');
				$('#clear-modal.modal').css('left','50px');
      }
	  else {
		  ...
		  //$("#area_1 .glyphicon-edit").trigger("click");
		  $('.svg-container').width('410');
		  $('.nav-tabs a[href="#tm_grid"]').tab('show');
				$('#clear-modal.modal').css('left','-445px');
	  }
  if(Imgnames){
    $.each(Imgnames, function (key, value) {
      var $pathForHTml2 = '/sites/default/files/projectChartImages/'+value.img_name;
    $.ajax({
      type: "POST",
      url: $pathForHTml2,
      success: function (result) {
        ...
        var divID = "tcluster_pbody_"+value.div_id;
        var img_num = 'tcluster_img_'+value.div_id;
        tclustermake(clusterJson , divID, value.div_id)
      ...
      $('.nav-tabs a[href="#tm_grid2"]').tab('show');
	});
...
/*	Drupal.behaviors.tm_analytics = {
		attach: function (context, settings) {
			$('.graphical-area .glyphicon-move', context).mousedown(function () {
				//$('.graphical-area .draggable-element').die();
				$('.graphical-area .draggable-element').attr('disabled','disabled');
			});
			$('.graphical-area .glyphicon-move', context).mouseup(function () {
				$('.graphical-area .draggable-element').attr('enabled','enabled');
			});
		}
	};*/
})(jQuery);
