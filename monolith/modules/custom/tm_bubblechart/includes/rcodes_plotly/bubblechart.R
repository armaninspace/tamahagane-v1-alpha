...
bSize        <- chartOptions$bubble_size
...
if (!is.data.frame(df)) {
  ...
  if (textCol == "" && cCol != "") {
     ...
     select(xCol, yCol, zCol, cCol, bCol)
     # coerce types
  ...
  if (cColType != "") {
    if (cColType == "factor" & (nrow(unique(dfFiltered[cCol])) < 3 | nrow(unique(dfFiltered[cCol])) > 10)) {
  ...
  if (nrow(dfFiltered) > 0) {
    ...
    tryCatch({
      ...
      siz <- as.numeric(dfFiltered[[bCol]])*as.numeric(bSize)
      if (textCol == "") {
        result <- try(
          chartGenerateTimeElapsed <- system.time (
            p <- plot_ly(x = dfFiltered[[xCol]], y = dfFiltered[[yCol]], z = dfFiltered[[zCol]],
                         type = "scatter3d", mode = "markers",color = dfFiltered[[cCol]], colors = "Paired",
                         marker = list(sizemode = "diameter", opacity = 0.5),sizes = c(10,50)*as.numeric(bSize),
                         size = siz, text = paste(paste0(bCol, ":"), dfFiltered[[bCol]])) %>%
        ...
        result <- try(
          chartGenerateTimeElapsed <- system.time (
            p <- plot_ly(x = dfFiltered[[xCol]], y = dfFiltered[[yCol]], z = dfFiltered[[zCol]],
                         type = "scatter3d", mode = "markers", color = dfFiltered[[cCol]], colors = "Paired",
                         marker = list(sizemode = "diameter", opacity = 0.5), sizes = c(10,50)*as.numeric(bSize),
                         size = siz, text = paste(paste0(bCol, ":"), dfFiltered[[bCol]], paste0("<br>",
                         textCol, ":"), dfFiltered[[textCol]])) %>%
                         config(displaylogo = F, collaborate = F) %>%
                         layout(scene = scene))
        , silent = TRUE)
      }
